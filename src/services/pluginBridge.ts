...
function getPlugin() {
  ...
  if (window.acquireVsCodeApi) {
    ...
    window.devpilot = {
      ...
      type: 'vscode',
      config: {
        theme: window.ideConfig.theme,
        locale: window.ideConfig.locale,
        username: window.ideConfig.username,
        loggedIn: window.ideConfig.loggedIn,
        // repo: {
        //   name: 'repo',
        //   embedding: false,
        // },
  ...
  window.devpilot = {
    ...
    handlers: [],
    sendToPlugin(message: PluginMessage) {
      logger.log('Message to plugin:', message);
    },
  };
